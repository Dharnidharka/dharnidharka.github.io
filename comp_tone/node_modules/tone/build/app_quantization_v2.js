...
var volumeMap = {
  ...
  function setWaveformFFT(){
    for(str in volumeMap){
      fft[str] = new Tone.Analyser("fft", 32);
      waveform[str] = new Tone.Analyser("waveform", 1024);
    }
  }
...
function playMidi(file) {
  MidiConvert.load(file, function(midi) {
    ...
    if(str in player){
      //console.log(fft);
      player[str].fan(fft, waveform).connect(cheby).start(time, 0, note.duration);
      if(str in volumeMap){
