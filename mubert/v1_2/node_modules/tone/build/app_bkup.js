var	midiPart, 
	...
	fft_data,
	wave_data,
	start_change_track = false,
	filter_all_flag = 0, 
	panner_all_flag = 0
	;
	
var	limiter = new Tone.Limiter(0).toMaster();
var	vol = new Tone.Volume(0).connect(limiter);
...
var MUBERT = function(){
	...
	this.getMubert = function(cfg){
	    if(!start_change_track){
		    start_change_track = true;
		    var useGenre = '';
	    ...
	    if(filter_tracks.length > 0){
	      for(var i=0; i<filter_tracks.length; i++){
	        switch(track){
	            ...
	            default: player[TRACK_LIST[filter_tracks[i]]].connect(new_filter);
	                      break;
	        }
	      }
	    }
	      ...
	      if(panner_tracks.length > 0){
	        for(var i=0; i<panner_tracks.length; i++){
	          switch(panner_tracks[i]){
	              ...
	              default: player[TRACK_LIST[panner_tracks[i]]].connect(panner);
	                        break;
	          }
	        }
	      }
	...
	this.setSchedule = function(){
	    ...
	    Tone.Transport.schedule(function(time){
	        ...
	        if(midiPart){
	          ...
	          $('.genre2').removeClass("quadrat").addClass('active');
	          start_change_track = false;
	        }
	      }, newtime);
	};
	...
	var playMidi = function(file) {
		  MidiConvert.load(file, function(midi) {
		    ...
		    if(str in player){
		      ...
		      player[str].fan(fft, waveform).connect(comp).start(time, 0, note.duration);
