...
function loadSingleFrame({ frame, baseObjectURL, baseTextureURL, assetType }) {
    ...
    return loadGeometries({ path, texturePath, baseObjectURL, baseTextureURL, assetType }).then(geometries => {
        if (frame.geometry) frame.geometry.forEach(geometry => geometry.dispose());
...
function loadGeometries({ path, texturePath, baseObjectURL, baseTextureURL, assetType }) {
    ...
    return new Promise((resolve, reject) => {
        ...
        switch (assetType) {
            case ".drc":
                dracoLoader.load(`${baseObjectURL}${path}`, function (geometry) {
                    geometry.texturePath = texturePath;
                    loadTextures([geometry]);
                });
                break;
            case ".obj":
                objLoader.load(`${baseObjectURL}${path}`, function ( object ) {
                    object.children[0].geometry.texturePath = texturePath;
                    loadTextures([object.children[0].geometry]);
				}, null, null );
                break;
            case ".gltf":
